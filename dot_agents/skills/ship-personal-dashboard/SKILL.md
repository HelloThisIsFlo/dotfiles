...
Before any mutation, give Flo one **ready-to-go handoff** that discloses the complete run. Wait for Flo to configure the requested Cloudflare route and Access hostname, then wait for an explicit go-ahead to that handoff.

That go-ahead authorizes only the actions listed in the handoff, including implementation, commits, pushes, package publication, fast-forward merge, deployment, state publication, verification, and merged remote-branch cleanup. Pause for an undisclosed expansion, a failed safety gate, missing authentication, or an external action that cannot be completed safely.
...
Tell Flo to configure both the Tunnel route and Access before replying. Confirm that an anonymous request to the hostname receives a Cloudflare Access redirect before any real state is deployed. A missing origin is expected until the Service exists.

## Execute after go-ahead

Read [release-and-verification.md](references/release-and-verification.md) completely, then execute the disclosed run persistently.
...
- Stop at the first failed gate and preserve the last valid ConfigMap, PVC contents, and deployed image.

## Finish

Return the protected URL and a short verified ledger: main SHA, image digest, Kubernetes health, published-state hash, Access behavior, repository visibility, package visibility, and cleanup.
