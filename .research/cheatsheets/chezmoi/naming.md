...
> **Gotcha: first decide whether the file is config or runtime state.** Authored secrets may belong in a password-manager template, but tool-owned authentication state should usually remain unmanaged. `~/.config/gh/hosts.yml` is the latter: GitHub CLI owns its account state and the OS credential store owns its OAuth token, so chezmoi manages `config.yml` and leaves `hosts.yml` unmanaged. Reserve `modify_` for genuinely shared files whose dynamic values cannot be predicted or stored — and accept that the script is inherently brittle (you're parsing and rewriting structured files in bash, handling edge cases like an empty file on a fresh machine, and hoping the tool's format doesn't change).
